...
def create_app(argument_agent: "ArgumentAgent", rebuttal_agent: "RebuttalAgent", summary_agent: "SummaryAgent"):
    app = FastAPI(
        title="DebateAgent",
        summary="A multi-agent system for the debate process, including argument, rebuttal, and summary agents."
    )
    ...
    async def get_prompt(input: AgentDebugInput):
        ...
        if position.lower() in ["positive", "正方"]:
            position = "pos"
        elif position.lower() in ["negative", "反方"]:
            position = "neg"
        else:
            raise HTTPException(
